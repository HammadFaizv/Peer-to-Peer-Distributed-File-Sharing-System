...
void Session::dispatch(uint16_t type, const std::string& payload) {
    ...
    // Words by claudy -
    // For the rest (file metadata). The pattern is always:
    //   1. reject if user_ is empty (except create_user / login)
    //   2. decode the payload with Buffer
    //   3. call the TrackerState method
    //   4. if it mutated state, sync_.record_and_replicate(type, payload)
    //   5. reply with the status and any response payload
    //
    // MSG_GET_FILE_META is the interesting one: reply with size, whole-file
    // hash, every piece hash, and the list of currently-online peers holding
    // the file (with their ip:port and bitfield).
